...
template<typename T>
inline bool neq(Buffer<T>& buf, std::vector<T>& vec){
	auto read_handle = buf.map_read();
	return !std::equal(read_handle.data(), read_handle.data() + buf.size(), vec.begin());
}
...
int main(){
	try{
		...
		*/
		...
		const int nsize = 5;
		buf.resize(nsize);
		if(buf.size() != nsize) throw std::runtime_error("Resize buffer");
